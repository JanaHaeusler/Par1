...
export default function useFormScoresCreate({
    newGameProfile,
    addGameProfile,
    updateVisibleForm }) {
    ...
    useEffect(() => setInputsScores(newGameProfile), [newGameProfile])
    useEffect(() => prepareInputValidation(newGameProfile), [newGameProfile])
    useEffect(() => prepareDirtyInputs(newGameProfile), [newGameProfile])
